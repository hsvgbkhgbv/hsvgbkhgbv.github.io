...
If an agent $k$  is selected by two initiators $p$ and $q$ **sequentially**, agent $k$ cooperate with $p$'s group firstly. The communication channel integrates their **thoughts**: $\{ \tilde{h}_{t}^{p}, ..., \tilde{h}_{t}^{k'} \} = g(h_{t}^{p}, ..., h_{t}^{k})$. Then agent $k$ communicate with $q$'s group: $\{ \tilde{h}_{t}^{q}, ..., \tilde{h}_{t}^{k''} \} = g(h_{t}^{q}, ..., \tilde{h}_{t}^{k'})$. The agent shared by different groups can fill in the information gap among individual groups. It can propagate the information throughout the groups and lead to the coordinated decisions among the whole groups.
