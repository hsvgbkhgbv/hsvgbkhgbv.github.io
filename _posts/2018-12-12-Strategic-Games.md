...
If the set $A_{i}$ of actions of every player $i$ is finite then the game is *finite*. We refer to an action profile $a = (a_{j})_{a}$ as an outcome, and denote the set $\times_{j \in N} A_{j}$ of outcomes by $A$. Therefore, each player may care not only about his own action but also about the actions taken by the other players, which is the feature that distinguishes a *strategic game* from a *decision problem*.
